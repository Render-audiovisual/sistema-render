...
test("distribuye cuotas sin superar cuatro publicaciones por día", () => {
  ...
  assert.ok([...totals.values()].every((total) => total <= 4));
});

test("respeta varios días preferidos y la ocupación fija", () => {
  const slots = buildEditorialSlots({ period: "2026-08", clients: [{ id: 7, nombre: "Bunker", activo: true, cuota_reels: 4, cuota_carruseles: 0, dias_reels: [2, 5] }], occupied: Array.from({ length: 4 }, (_, id) => ({ id, fecha_programada: "2026-08-04" })) });
