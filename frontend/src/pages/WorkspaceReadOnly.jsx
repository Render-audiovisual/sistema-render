...
function TasksView({ tasks, users, query, area, setArea, legacyBase = "" }) {
...
function ClientsView({ clients, tasks, query, legacyBase = "" }) {
...
function TeamView({ users, tasks, query, legacyBase = "" }) {
...
export function WorkspaceStagingLogin() {
  ...
  return <main className="ros-staging-login"><section><div className="ros-staging-brand"><span>R</span><strong>RENDER</strong><b>OS</b></div><div className="ros-readonly-pill">🔒 STAGING · SOLO LECTURA</div><h1>Validación con datos reales</h1><p>Ingresá con la misma cuenta del sistema. La sesión se conecta a <strong>sistema.rendercorrientes.com</strong> y esta vista no permite modificar tareas, clientes ni usuarios.</p><form onSubmit={submit}><label>Usuario<input autoComplete="username" value={usuario} onChange={(event) => setUsuario(event.target.value)} required autoFocus/></label><label>Contraseña<input type="password" autoComplete="current-password" value={contrasena} onChange={(event) => setContrasena(event.target.value)} required/></label>{error && <div className="ros-login-error">{error}</div>}<button type="submit" disabled={loading}>{loading ? "Ingresando…" : "Entrar al staging"}</button></form><a href="https://sistema.rendercorrientes.com/login">Volver al sistema actual ↗</a></section></main>;
}

export function WorkspaceReadOnlyPage({ path, sesion, staging = false }) {
  const requestedSection = new URLSearchParams(window.location.search).get("section");
  const section = staging && ["tasks", "clients", "team"].includes(requestedSection)
    ? requestedSection
    : SECTIONS[path] || "tasks";
  ...
  const workspaceTarget = (target) => staging
    ? `/workspace-staging/?section=${target}`
    : `/workspace/${target === "tasks" ? "tareas" : target === "clients" ? "clientes" : "equipo"}`;
  ...
  useEffect(() => {
    ...
    const redirectStagingLinks = (event) => {
      ...
      if (workspaceSections[href]) {
        event.preventDefault();
        window.location.href = workspaceTarget(workspaceSections[href]);
      } else if (href?.startsWith("/")) {
        event.preventDefault();
        window.location.href = `${legacyBase}${href}`;
      }
    };
