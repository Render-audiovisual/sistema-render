...
export function WorkspaceReadOnlyPage({ sesion }) {
  ...
  return <div className="render-workspace"><Toast toast={toast} onClose={() => setToast(null)}/><main className="ros-main"><header className="ros-topbar"><div><span>RENDER</span><b>/</b><strong>{title}</strong></div><input className="ros-top-search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={placeholder}/></header>{loading || error ? <LoadingState error={error}/> : <TasksView tasks={tasks} totalTasks={totalTasks} loadingMore={loadingMore} onLoadMore={loadMoreTasks} users={users} clients={clients} query={query} area={area} setArea={setArea} sesion={sesion} onCreate={createTask} onUpdate={updateTask} onDelete={deleteTask}/>}</main></div>;
}
