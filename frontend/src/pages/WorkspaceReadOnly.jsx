  ...
  return <div className="ros-drawer-backdrop" onClick={closeDetail}>
    <aside className="ros-drawer ros-task-workspace" onClick={(event) => event.stopPropagation()}>
      <header className="ros-task-workspace-header">
        <div className="ros-task-header-top"><AreaBadge task={task}/><button type="button" aria-label="Cerrar tarea" onClick={closeDetail}>×</button></div>
        <div className="ros-task-client">{task.cliente_nombre || "SIN CLIENTE"}</div>
        {editing ? <input className="ros-title-input" value={draft.titulo || ""} onChange={(event) => setDraft({ ...draft, titulo: event.target.value })}/> : <h2>{task.titulo}</h2>}
        <div className="ros-task-header-meta"><span className="ros-task-status"><i style={{ background: status.color }}/> {status.label}</span><span><Avatar person={person} name={task.asignado_a}/>{task.asignado_a || "Sin asignar"}</span><span>Entrega {formatDate(task.fecha_vencimiento)}</span></div>
      </header>
      <div className="ros-drawer-body ros-task-workspace-body">
        ...
        {!editing && <section className="ros-work-block ros-objective-block"><div className="ros-block-heading"><div><span>01</span><h3>Qué hay que lograr</h3></div><small>Objetivo</small></div><p>{objectiveText}</p></section>}
        ...
        <section className="ros-task-context-card">
          <div className="ros-task-context-title"><span>Contexto de la tarea</span>{isAdmin && !editing && <button className="ros-task-edit-button" type="button" onClick={startEditing}>Editar</button>}</div>
          <div className="ros-task-context-person"><Avatar person={person} name={task.asignado_a}/><div><strong>{task.asignado_a || "Sin asignar"}</strong><span>Responsable</span></div></div>
          <dl><div><dt>Estado</dt><dd><i style={{ background: status.color }}/>{status.label}</dd></div><div><dt>Entrega</dt><dd>{formatDate(task.fecha_vencimiento)}</dd></div><div><dt>Prioridad</dt><dd>{task.prioridad || "Media"}</dd></div><div><dt>Cliente</dt><dd>{task.cliente_nombre || "Sin cliente"}</dd></div></dl>
        </section>
        <section className="ros-latest-progress">{lastActivity ? <><strong>Último avance</strong><span>{lastActivity.contenido.replace(/^\[Actividad\]\s*/, "")}</span><time>{formatDateTime(lastActivity.created_at)}</time></> : <><strong>Situación actual</strong><span>{status.label} · {task.asignado_a || "Sin responsable"}</span></>}</section>
...
function TaskCard({ task, users, today, onOpen, onMove, canMove }) {
  const person = personForTask(task, users);
