  ...
  return <div className="ros-drawer-backdrop" onClick={closeDetail}>
    <aside className="ros-drawer ros-task-workspace" onClick={(event) => event.stopPropagation()}>
      <header className="ros-task-workspace-header">
        ...
        <div className="ros-task-header-meta"><span className="ros-task-status"><i style={{ background: status.color }}/> {status.label}</span><span><Avatar person={person} name={task.asignado_a}/>{task.asignado_a || "Sin asignar"}</span><span>Entrega {formatDate(task.fecha_vencimiento)}</span>{isAdmin && !editing && <button className="ros-task-edit-button" type="button" onClick={startEditing}>Editar tarea</button>}</div>
      </header>
