...
function TasksView({ tasks, totalTasks, loadingMore, onLoadMore, users, clients, query, area, setArea, sesion, onCreate, onUpdate, onDelete }) {
  ...
  return <><section className="ros-page">
    <div className="ros-title-row"><div><div className="ros-eyebrow">CENTRO OPERATIVO</div><h1>Tareas</h1><p>Creación, responsables, estados, comentarios y actividad sobre la base real.</p></div>{isAdmin && <button className="ros-primary-button" type="button" onClick={() => setCreating(true)}>+ Nueva tarea</button>}</div>
