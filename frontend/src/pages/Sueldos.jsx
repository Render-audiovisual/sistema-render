    ...
    {!loading && !error && finance && <>
      ...
      <section className="finance-metrics" aria-label="Resumen financiero automático">
        <article style={{background: '#f0f4f8', borderColor: '#d4dce5'}}>
          <span>📊 RESULTADO</span>
          <strong style={{fontSize: '28px', color: finance.resultadoARS >= 0 ? '#b5fc00' : '#ff6b6b'}}>{ars.format(finance.resultadoARS)}</strong>
          <small style={{color: 'var(--muted)', marginTop: '4px'}}>Margen: <strong>{margen}%</strong></small>
          {resultadoVariation && <small style={{color: resultadoVariation.isNegative ? '#2d5a4e' : '#a7322a', fontSize: '11px'}}>{resultadoVariation.isNegative ? '↓' : '↑'} {resultadoVariation.percent}%</small>}
        </article>
        <article style={{background: '#fff7e9', borderColor: '#e7c99e'}}>
          <span>💵 FACTURACIÓN</span>
          <strong style={{fontSize: '28px', color: '#b5fc00'}}>{ars.format(finance.facturacion)}</strong>
        ...
        <article style={{background: '#f8f0f0', borderColor: '#e5d4d4'}}>
          ...
          <strong style={{fontSize: '28px', color: '#c57575'}}>{ars.format(finance.gastosFijosARS)}</strong>
